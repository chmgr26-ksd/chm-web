...
// motion: true → 정적 이미지 외에 모션 GIF·영상(mp4/webm)도 허용(원본 보존 저장, Hero에서 <video> 렌더).
// internal: true → 관리자 UI에 안 보이는 내부 슬롯. landing-hero-poster는 영상 업로드 시
//   클라이언트가 캡처한 첫 프레임(정지 이미지)을 저장 → <video poster>로 사용(로딩 중 빈 화면 방지).
export const SITE_IMAGE_SLOTS = {
  ...
  'landing-hero':        { default: 'landing/hero.jpg',   label: '랜딩 · 히어로 이미지',  page: '/', motion: true },
  'landing-hero-poster': { default: 'landing/hero.jpg',   label: '랜딩 히어로 포스터',    page: '/', internal: true },
...
async function readContact() {
  ...
  return {
    ...
    reviewFormUrl: s?.reviewFormUrl || REVIEW_FORM_URL,
  };
}
...
// 슬롯별 현재 저장 mimeType(key→mime). DB 행 없으면 미포함(=기본 public 이미지).
async function readImageMimes() {
  let rows = [];
  try {
    rows = await prisma.siteImage.findMany({ select: { key: true, mimeType: true } });
  } catch {
    rows = [];
  }
  const map = {};
  for (const r of rows) map[r.key] = r.mimeType;
  return map;
}

export const getSiteImageMimes = unstable_cache(readImageMimes, ['site-image-mimes'], { tags: ['site-images'] });
