...
// 구글맵 임베드 URL — 좌표("위도,경도")면 해당 지점에 핀을 정확히 고정하고,
// 그 외엔 주소 문자열을 지오코딩. 좌표 우선(도로명 지오코딩 오차 회피).
export function mapEmbedUrl(input) {
  const val = String(input || DEFAULTS.geo || DEFAULTS.address).trim();
  const isGeo = /^-?\d{1,3}(?:\.\d+)?\s*,\s*-?\d{1,3}(?:\.\d+)?$/.test(val);
  const q = encodeURIComponent(isGeo ? val.replace(/\s+/g, '') : val);
  // z=17로 건물이 보이게 확대.
