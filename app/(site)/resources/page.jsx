        ...
        <Container size="lg" className="py-16">
          {resources.length === 0 ? (
            ...
            <ul className="flex flex-col gap-3">
              {resources.map((doc) => {
                const dl = `/api/resources/${doc.id}/download`;
                return (
                  <li key={doc.id} className="flex items-start gap-4 rounded-chm-lg border border-border bg-surface p-4 transition hover:border-primary hover:bg-surface-warm">
                    ...
                    <div className="min-w-0 flex-1">
                      <a href={dl} className="block truncate text-body-lg font-bold text-ink-850 hover:text-primary hover:underline">{doc.title}</a>
                      {doc.description && <RichText html={doc.description} className="mt-1 text-body-sm text-ink-600" />}
                      <div className="mt-1 text-caption text-ink-500">{doc.filename} · {fmtSize(doc.size)}</div>
                    </div>
                    <a href={dl} className="flex-none self-center rounded-chm-md border border-border px-3 py-2 text-body-sm font-semibold text-ink-700 hover:border-primary hover:text-primary">내려받기 ↓</a>
                  </li>
                );
              })}
            </ul>
