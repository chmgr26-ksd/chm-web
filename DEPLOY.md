...
### 에러 모니터링(Sentry, 선택 — 브라우저 전용)
경량화를 위해 `@sentry/browser`만 사용합니다(무거운 서버 opentelemetry 트리 제거 → 취약점·용량 최소화). **브라우저(사용자 화면) 오류**를 수집하며, 서버 오류는 Hostinger 런타임 로그로 확인합니다. **DSN 미설정 시 완전 비활성**.

| 이름 | 값 | 용도 |
|---|---|---|
| `NEXT_PUBLIC_SENTRY_DSN` | Sentry 프로젝트 DSN | 브라우저 오류 전송(빌드 시 주입 → **설정 후 재배포 필요**) |

> sentry.io 무료 가입 → 프로젝트(플랫폼: Browser/JavaScript) 생성 → DSN 복사 → 위 변수 등록 → 재배포.
> (이전의 서버용 `SENTRY_DSN`은 더 이상 사용하지 않습니다.)
